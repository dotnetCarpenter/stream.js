...
describe('standard functional functions', function () {
  ...
  it('drops items from the front of the stream', function () {
    var oldComment = {};
    var oldestComment = {};
    var newComment = {};
    var newestComment = {};
    ...
    /* TODO: delete
    var testData = ["a", null, "b", null, "c"];
    //var testData = ["a", "b", "c"];
    var testStream = convertToStream(testData).drop(4);
    expect(testStream.length()).toBe(1);
    expect(testStream.head()).toBe("c");
    expect(testStream.item(0)).toBe("c");
    testStream.print();
    function convertToStream(list) {
      var x = list[0],
          xs= list.slice(1);
      return new Stream(x, function() { return convertToStream(xs); })
    }*/
  })
